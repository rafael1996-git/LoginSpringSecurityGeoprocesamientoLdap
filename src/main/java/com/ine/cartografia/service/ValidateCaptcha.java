...
import com.ine.cartografia.filter.CaptchaResponse;
...
public class ValidateCaptcha {
	protected static final Log logger = LogFactory.getLog(ValidateCaptcha.class.getName());
    ...
    public boolean validateCaptcha(final String captchaResponse) {
    	...
    	logger.info("Going to validate the captcha response = {}"+ captchaResponse);
        ...
        if (Objects.nonNull(apiResponse) && apiResponse.isSuccess()) {
        	logger.info("Captcha API response = {}"+ apiResponse.toString());
            return true;
        } else {
            return false;
        }
